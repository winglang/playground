...
2. ***Inflight:*** Code that runs at runtime to handle the application logic (handle API requests, process queue messages, etc). It can run inside a serverless function (lambda), a container, a VM or a physical server.

You are probably familiar with solutions in which code of the two phases is separated into different languages, frameworks and compilers.
