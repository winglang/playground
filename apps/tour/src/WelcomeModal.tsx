...
export const WelcomeModal = ({visible,loading = false, onClose}: WelcomeModalProps) => {
  return (
    <div className={classNames(
      "absolute inset-0 bg-[#293443] z-50 w-full h-full pt-10 flex flex-col font-sans",
      {"hidden": !visible}
      )}>
      <div className="space-y-10 text-[#BDCECC] grow">
        <h1 className="text-3xl block font-bold">Welcome to the Winglang tutorial!</h1>
      ...
      <div className="flex justify-end">
        <button
          ...
          "text-xs px-4 py-2 cursor-pointer",
            loading && "text-[#2AD5C1] border border-[#2AD5C1]"
          )}
          onClick={onClose}
          disabled={loading}
        >
