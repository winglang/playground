...
const CodeRow = ({loading, width} : {loading?: boolean, width: string}) => {
  return (
    <div className="flex gap-x-[25px] h-5">
      <div className={classNames("w-5 bg-gray-400 dark:bg-gray-700 rounded-sm shrink-0", loading && "animate-pulse")}/>
      <div className={classNames("bg-gray-400 dark:bg-gray-700 rounded-sm shrink-0", loading && "animate-pulse")} style={{width: width}}/>
    </div>
  );
}
