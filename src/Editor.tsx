...
export const ReactMonacoEditor: React.FC<EditorProps> = ({
    ...
    const onFileChange = async (event: any, data: any) => {
      console.log(data)
      const file =  data.text.split('/').pop()!;
      const ext = file.split('.').pop()!;
      setLanguageContext({ file: data.text, path: `source.${file.split('.').pop()}`, language: supportedLanguages(ext) })
    ...
    return (
      <div className='flex flex-col h-full'>
        ...
        {modalVisibility && <Modal setModalVisibility={setModalVisibility} title='Compilation Output' onDownload={onDownload}>
          ...
          <div className="hidden">{currentExample.text}</div>
        </Modal>}
      </div>
    );
};
